...
class TranscriptionProcessor:
    ...
    def convert_to_mono(self):
        ...
        audio_mono.export("uploaded_files/mono.wav", format="wav")

    def reduce_noise(self):
        rate, data = wavfile.read("uploaded_files/mono.wav")
        reduced_noise = nr.reduce_noise(y=data, sr=rate)
        wavfile.write("uploaded_files/mono_reduced.wav", rate, reduced_noise)

    def load_whisper_model(self):
        subprocess.call(['ffmpeg', '-i', 'uploaded_files/mono_reduced.wav', 'uploaded_files/mono.wav', '-y'])
        result = self.model.transcribe("uploaded_files/mono_reduced.wav")
        self.segments = result["segments"]

    def transcribe_audio(self):
        frames, rate, duration = self.get_audio_info('uploaded_files/mono_reduced.wav')
    ...
    def segment_embedding(self, segment, duration):
        ...
        waveform, _ = self.audio.crop('uploaded_files/mono_reduced.wav', clip)
    ...
    def save_transcription(self):
        with open("output_files/transcript.txt", "w", encoding="utf-8") as f:
            for (i, segment) in enumerate(self.segments):
